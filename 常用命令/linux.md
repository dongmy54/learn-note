...
2. 
  ...
  - ctrl + z
  - 输入bg
  (如果要重新调到前台来，jobs查看编号 然后 fg 1 -假设编号是1)
3. nohup your_command & 以守护进程（关掉终端不影响）方式执行

### 服务器磁盘空间满
```bash
df -h  # 查看当前空间占用情况
df -h /home/user # 查看目录所属挂载点使用量信息

# 列出当前目录下所有文件/文件夹占用大小并排序,取最大的十个
...
### shell脚本
1. ${Name} 华括号是变量引用;$(commond) 命令执行
2. 函数定义functionName() {};始终无参数；
  - 传参通过 functionName param1 params2 写入；
  - 内部通过$1 方式获取 
3. 变量推荐写法，外层加双引号比如 "${HOST}"
  区别,假设dir="ab bc ed"
  -`mkdir ${dir}` 会自动展开，创建三个目录
  -`mkdir "${dir}"` 当作整理创建一个目录
...
scp ent_pro:'/home/kuban/xh*' ./Downloads/客户支持/
...
grep -A 10 -B 10 self.product_klass.find_by /Users/dongmingyan/t.rb  # 过滤匹配内容的前后10行 A-after B-before
grep -C 10 self.product_klass.find_by /Users/dongmingyan/t.rb # 与上等效 前后10行
grep -rl . -e 'iot.v2.kuban.io' # 只找出对应的文件
grep -rn . -e 'iot.v2.kuban.io' # 找出当前目录下所有文件中 包含iot.v2.kuban.io的文件并打印行号
# -r 递归
# -n 行号
# -l 列出文件名
...
# ============================= 网络 ==========================
route get baidu.com 查看走的是哪个网关/网卡
...
hey -c 3 -q 4 -z 1s \
  ...
  -H 'AccessToken: dx abc' \
  -m 'post' \
  -d '{
    "loginType": "pwd",
    "identity": "18200375876",
    "info": "dmy067",
    "fid": "string",
    "ip": "string"
  }'\
  'http://192.168.15.188:82/df/dxin/pc/auth/login'
```
